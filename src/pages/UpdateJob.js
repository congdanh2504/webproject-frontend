...
const UpdateJob = () => {
  ...
  const submit = (e) => {
    if (title && nameJob && description && category && duration && salary
      && province && ward && district && street) {
      updateJob(setError, id.id, title, nameJob, description, category, salary, duration, province,
        district, ward, street)

      setError(null)
      setSuccess('Update successfullly! Redirecting...')
      e.preventDefault();
      setTimeout(() => {
        history.push(`/jobs/job-details/${id.id}`)
      }, 2000)
    } else {
              ...
              <div class="card-body">
                <form>
                  <div class="row form-row">
                    ...
                    <InputTag type='text' title="Title" value={title ? title : job?.title} onChange={changeTitle} />
                    <div class="col-12 col-md-6">
                      <div class="form-group">
                    ...
                    <InputTag type='text' title="Carrier" value={nameJob ? nameJob : job?.nameJob} onChange={changeNameJob} />
                    <InputTag type='date' title="Duration" onChange={changeDuration} />
                    <InputTag type='number' title="Salary (USD)" value={salary ? salary : job?.salary} onChange={changeSalary} />
                  </div>
                  ...
                  <div class="row form-row">
                    <TextAreaTag
                      ...
                      value={description ? description : job?.description}
                    />
                  </div>
