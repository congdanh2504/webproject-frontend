...
function EmployerRegisterForm() {
  ...
  const submit = async () => {
    setLoading(true)
    await employerRegister(email, userName, password, rePassword, setEmailError, setUserNameError, setPasswordError, setRePasswordError, history, toast)
    setLoading(false)
  }

  const handleGG = (param) => {
    registerWithGG(param.tokenId, setPasswordError, history, "Employer")
  }
